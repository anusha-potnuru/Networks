...
#include <netinet/ip.h> /* for upd header */ 
#include <netinet/udp.h> /* for ipv4 header */ 
...
// #define LISTEN_IP "127.0.0.1"" #define LISTEN_IP "0.0.0.0"
int main(){
	int rawfd, udpfd;
	struct sockaddr_in saddr_raw, saddr_udp; struct sockaddr_in raddr;
	...
	if(pid == 0)
	{
		...
		int iphdrlen = sizeof(hdrip); int udphdrlen = sizeof(hdrudp);
		...
		saddr_raw.sin_port = htons(LISTEN_PORT); saddr_raw.sin_addr.s_addr = INADDR_ANY; //inet_addr(LISTEN_IP); saddr_raw_len = sizeof(saddr_raw);
		...
		while(1)
		{
			...
			hdrudp = *((struct udphdr *) (msg + iphdrlen)); if(hdrudp.uh_dport != saddr_raw.sin_port)
							continue;
						msg[msglen] = 0;
			printf("RAW socket: ");
			...
			printf("\nRAW socket: \tUdp sport: %d, dport: %d", ntohs(hdrudp.uh_sport), ntohs(hdrudp.uh_dport));
	...
	else
	{
		...
		saddr_udp.sin_port = htons(LISTEN_PORT); saddr_udp.sin_addr.s_addr = INADDR_ANY; //inet_addr(LISTEN_IP); saddr_udp_len = sizeof(saddr_udp);
